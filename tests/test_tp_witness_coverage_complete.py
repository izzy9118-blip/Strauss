...
class CompleteTPWitnessCoverageTests(unittest.TestCase):
    ...
    def test_seven_witness_only_items_remain_noncertified_and_unstudied(self):
        corpus = load_yaml("corpus/index.yaml")
        complete = {"CORPUS-SRC-102", "CORPUS-SRC-103", "CORPUS-SRC-105", "CORPUS-SRC-108", "CORPUS-SRC-109", "CORPUS-SRC-111", "CORPUS-SRC-113", "CORPUS-SRC-116", "CORPUS-SRC-101", "CORPUS-SRC-104", "CORPUS-SRC-106", "CORPUS-SRC-107"}
        witness_only = [
            x
            for x in corpus["source_entities"]
            if x["source_id"].startswith("CORPUS-SRC-1") and x["source_id"] not in complete
        ]
        self.assertEqual(len(witness_only), 7)
