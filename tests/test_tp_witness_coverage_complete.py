...
class CompleteTPWitnessCoverageTests(unittest.TestCase):
    def test_all_nineteen_predecessor_sources_have_reviewed_witnesses(self):
        ...
        self.assertEqual(corpus["coverage"]["theologico_political_independent_item_studies_registered"], 5)
    ...
    def test_fourteen_witness_only_items_remain_noncertified_and_unstudied(self):
        corpus = load_yaml("corpus/index.yaml")
        complete = {"CORPUS-SRC-102", "CORPUS-SRC-103", "CORPUS-SRC-105", "CORPUS-SRC-109", "CORPUS-SRC-111"}
        witness_only = [
            x
            for x in corpus["source_entities"]
            if x["source_id"].startswith("CORPUS-SRC-1") and x["source_id"] not in complete
        ]
        self.assertEqual(len(witness_only), 14)
