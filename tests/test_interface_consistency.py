...
class InterfaceConsistencyTests(unittest.TestCase):
    ...
    def test_nineteen_identity_nineteen_witness_eight_study_language_matches(self) -> None:
        manifest = load_yaml("manifest.yaml")
        audit = load_yaml("audits/operational-completeness.yaml")
        mapping = load_yaml("migrations/lean-operational-interface.yaml")
        corpus = load_yaml("corpus/index.yaml")
        ...
        self.assertEqual(manifest_state["independent_sequential_study_count"], 8)
        self.assertEqual(audit_state["independently_reconstructed_count_within_this_sequence"], 8)
        self.assertEqual(mapping_state["independent_sequential_study_count"], 8)
        ...
        self.assertEqual(manifest_state["remaining_without_independent_sequential_study"], 11)
        self.assertEqual(mapping_state["remaining_without_independent_sequential_study"], 11)
        self.assertEqual(
    ...
    def test_priority_schedule_marks_witness_acquisition_complete_and_spinoza_treatise_complete_genesis_study_next(self) -> None:
        ...
        self.assertEqual(schedule["selection"]["completed_study_ids"], ["JA-STUDY-001", "COHEN-STUDY-001", "TALMON-STUDY-001", "SPINOZA-PREFACE-STUDY-001", "SPINOZA-TREATISE-STUDY-001", "GENESIS-STUDY-001", "PERSECUTION-INTRO-STUDY-001", "HOBBES-PREFACE-STUDY-001"])
        self.assertEqual(schedule["termination"]["reviewed_item_witness_registration"], "COMPLETE_19_OF_19")
        self.assertEqual(schedule["termination"]["independent_sequential_reconstruction"], "INCOMPLETE_8_OF_19")
        self.assertEqual(schedule["termination"]["next_item_witness"], "NONE")
        self.assertEqual(schedule["termination"]["next_item_study"], "CORPUS-SRC-101")
