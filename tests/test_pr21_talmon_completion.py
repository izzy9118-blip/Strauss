...
class TalmonIntegrationCompletionTests(unittest.TestCase):
    def test_operational_records_are_synchronized(self) -> None:
        ...
        self.assertEqual(manifest["identity"]["version"], "1.10.0")
        self.assertEqual(audit["identity"]["version"], "2.8.0")
        self.assertEqual(mapping["identity"]["version"], "1.10.0")
        self.assertEqual(process["identity"]["version"], "1.12.0")
        self.assertEqual(schedule["identity"]["version"], "1.10.0")
        self.assertEqual(corpus["identity"]["version"], "1.14.0")
        self.assertEqual(findings["identity"]["version"], "1.6.0")
        ...
        self.assertEqual(state["independent_sequential_study_count"], 6)
        self.assertEqual(state["remaining_without_reviewed_item_witness"], 0)
        self.assertEqual(state["remaining_without_independent_sequential_study"], 13)
        self.assertEqual(
            state["completed_study_ids"],
            ["COHEN-STUDY-001", "JA-STUDY-001", "TALMON-STUDY-001", "SPINOZA-PREFACE-STUDY-001", "SPINOZA-TREATISE-STUDY-001", "GENESIS-STUDY-001"],
        )
