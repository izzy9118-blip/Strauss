...
class TalmonIntegrationCompletionTests(unittest.TestCase):
    def test_operational_records_are_synchronized(self) -> None:
        ...
        self.assertEqual(manifest["identity"]["version"], "1.15.0")
        self.assertEqual(audit["identity"]["version"], "3.3.0")
        self.assertEqual(mapping["identity"]["version"], "1.15.0")
        self.assertEqual(process["identity"]["version"], "1.17.0")
        self.assertEqual(schedule["identity"]["version"], "1.15.0")
        self.assertEqual(corpus["identity"]["version"], "1.19.0")
        self.assertEqual(findings["identity"]["version"], "1.11.0")
        ...
        self.assertEqual(state["independent_sequential_study_count"], 11)
        self.assertEqual(state["remaining_without_reviewed_item_witness"], 0)
        self.assertEqual(state["remaining_without_independent_sequential_study"], 8)
        self.assertEqual(
            state["completed_study_ids"],
            ["COHEN-STUDY-001", "JA-STUDY-001", "TALMON-STUDY-001", "SPINOZA-PREFACE-STUDY-001", "SPINOZA-TREATISE-STUDY-001", "GENESIS-STUDY-001", "PERSECUTION-INTRO-STUDY-001", "HOBBES-PREFACE-STUDY-001", "PROGRESS-RETURN-STUDY-001", "HUSIK-PREFACE-STUDY-001", "FREUD-MOSES-STUDY-001"],
        )
