...
class TalmonIntegrationCompletionTests(unittest.TestCase):
    def test_operational_records_are_synchronized(self) -> None:
        ...
        self.assertEqual(manifest["identity"]["version"], "1.9.0")
        self.assertEqual(audit["identity"]["version"], "2.7.0")
        self.assertEqual(mapping["identity"]["version"], "1.9.0")
        self.assertEqual(process["identity"]["version"], "1.11.0")
        self.assertEqual(schedule["identity"]["version"], "1.9.0")
        self.assertEqual(corpus["identity"]["version"], "1.13.0")
        self.assertEqual(findings["identity"]["version"], "1.5.0")
        ...
        self.assertEqual(state["independent_sequential_study_count"], 5)
        self.assertEqual(state["remaining_without_reviewed_item_witness"], 0)
        self.assertEqual(state["remaining_without_independent_sequential_study"], 14)
        self.assertEqual(
